...
// MARK: - CardFlipAnimator tests (platform-independent, run on Linux CI)

final class CardFlipAnimatorTests: XCTestCase {
    ...
    /// Back face scale: returns 1.0 for degrees <= 90 (back not visible), grows to 1.0 at 180°.
    func testFlipAnimatorBackFaceScale() {
        XCTAssertEqual(CardFlipAnimator.backFaceScale(at: 90), 1.0, accuracy: 0.001,
                       "Returns safe default 1.0 when back face not yet visible")
    ...
    final class InteractiveCardTests: XCTestCase {
        ...
        func testInteractiveCardReplace() {
            ...
            // replace uses async dispatch; we verify it compiles and runs without errors
            XCTAssertNotNil(interactiveCard)
        }
    }
