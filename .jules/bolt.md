## 2026-08-05 - Precompute Binomial Coefficient Pointers and Unroll Fast Möbius Transform
**Learning:** Pre-calculating row pointer offsets for combinatorial lookup tables (like the binomial chooseTable) outside of ultra-frequent inner loops completely eliminates millions of redundant address multiplications and pointer arithmetic calculations on the hot path. Additionally, fully unrolling fixed-size Fast Möbius Transform (FMT) loops (such as $2^5 = 32$ element subset transforms) avoids all nested loop control, index calculation, and shift logic. Together, these optimizations deliver a massive ~15.35% overall speedup in exact whole-pay-table return-to-player (RTP) analysis.
**Action:** Always hoist table stride multiplications and pointer address offsets outside of frequent inner loops, and unroll fixed-size combinatorial transforms to optimize hardware pipeline efficiency.
