...
struct HandOutcomeArrays {
    ...
    /// and precomputed card row pointers to eliminate stride multiplications.
    @inline(__always)
    func payout(
        ...
        var position = 0
        if mask & 0b00001 != 0 {
            position += 1; index += cardRowPtrs.0[position]
        }
        if mask & 0b00010 != 0 {
            position += 1; index += cardRowPtrs.1[position]
        }
        if mask & 0b00100 != 0 {
            position += 1; index += cardRowPtrs.2[position]
        }
        if mask & 0b01000 != 0 {
            position += 1; index += cardRowPtrs.3[position]
        }
        if mask & 0b10000 != 0 {
            position += 1; index += cardRowPtrs.4[position]
        }
