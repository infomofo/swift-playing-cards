...
struct HandOutcomeArrays {
    ...
    // ⚡ Bolt Optimization: Accept precomputed row pointers for cards (choosePtr + cardIndex * stride)
    // to eliminate stride multiplications and addition operations inside the 32-mask loop.
    @inline(__always)
    func payout(
        ...
        var position = 0
        if mask & 0b00001 != 0 {
            position += 1; index += cardPtrs.0[position]
        }
        if mask & 0b00010 != 0 {
            position += 1; index += cardPtrs.1[position]
        }
        if mask & 0b00100 != 0 {
            position += 1; index += cardPtrs.2[position]
        }
        if mask & 0b01000 != 0 {
            position += 1; index += cardPtrs.3[position]
        }
        if mask & 0b10000 != 0 {
            position += 1; index += cardPtrs.4[position]
        }
