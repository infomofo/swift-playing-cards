...
struct HandOutcomeArrays {
    ...
    /// and precomputed card pointers to eliminate stride multiplications in hot paths.
    @inline(__always)
    func payout(
        forSubsetMask mask: Int,
        card0Ptr: UnsafePointer<Int>,
        card1Ptr: UnsafePointer<Int>,
        card2Ptr: UnsafePointer<Int>,
        card3Ptr: UnsafePointer<Int>,
        card4Ptr: UnsafePointer<Int>,
        ...
        var position = 0
        if mask & 0b00001 != 0 {
            position += 1; index += card0Ptr[position]
        }
        if mask & 0b00010 != 0 {
            position += 1; index += card1Ptr[position]
        }
        if mask & 0b00100 != 0 {
            position += 1; index += card2Ptr[position]
        }
        if mask & 0b01000 != 0 {
            position += 1; index += card3Ptr[position]
        }
        if mask & 0b10000 != 0 {
            position += 1; index += card4Ptr[position]
        }
