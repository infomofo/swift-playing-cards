...
struct HandOutcomeArrays {
    ...
    ///
    /// ⚡ Bolt Optimization: Accepts pre-computed card row pointers (`r0...r4`), avoiding per-mask
    /// stride multiplications (`card * chooseTableStride`) inside the hot 32-mask loop.
    @inline(__always)
    func payout(
