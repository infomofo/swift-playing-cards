...
public enum PayTableAnalyzer {
    /// ⚡ Bolt Optimization: Precompute 32 reciprocals indexed directly by holdMask (0...31).
    /// Direct indexing avoids calling `nonzeroBitCount` population count instructions and indirect
    /// array lookups on every mask inside the hot evaluation loop (83+ million calls).
