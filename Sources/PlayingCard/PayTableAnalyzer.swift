...
public enum PayTableAnalyzer {
    ...
    /// Avoids calling `nonzeroBitCount` population count instructions and array lookups inside hot loops.
    private static let reciprocalCompletionsForMask: [Double] = (0 ..< 32).map { holdMask in
    ...
    public static func overallReturn(payTable: PayTable) -> Double {
        ...
        reciprocalCompletionsForMask.withUnsafeBufferPointer { reciprocalBuf in
    ...
    private static func bestHoldEV(
        ...
        // ⚡ Bolt Optimization: Precompute row pointers into chooseTable for each card in hand.
        // This avoids 5 multiplications per subset (160 multiplications per hand, over 400M total)
        // inside payout forSubsetMask loop.
        ...
        // Unrolled Fast Möbius Transform (FMT) in-place:
        // step 0: stepSize = 1
